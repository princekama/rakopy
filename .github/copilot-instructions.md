...
- **Mocked TCP in tests** — Tests never open real connections. They use `unittest.mock.AsyncMock` for `StreamReader`/`StreamWriter` and helper functions from `tests/helpers.py` (`make_reader`, `make_writer`, `json_line`), which are imported and used by `tests/conftest.py`. The pattern is: patch `_reconnect` as a no-op, set `hub._reader`/`hub._writer` to mocks, then call the method under test.
